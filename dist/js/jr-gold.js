...
Object.entries(scores).forEach(([day, games], index) => {
  ...
  const avg = games.length ? total / games.length : 0;
  ...
  resultsContainer.innerHTML += `
    <div style="margin-bottom: 20px;">
      ...
      <p style="color: #c5c6c7;">Scores: ${games.join(", ") || "No scores"}</p>
      <p style="color: #c5c6c7;">Average: ${avg.toFixed(2)}</p>
      <p style="color: #c5c6c7;">+/- from 200: ${plusMinus > 0 ? "+" : ""}${plusMinus}</p>
      <p style="color: #c5c6c7;">Total: ${total}</p>
    </div>
  `;
});

const overallAverage = totalGames ? totalAll / totalGames : 0;
...
resultsContainer.innerHTML += `
  ...
  <div style="margin-top: 20px;">
    ...
    <p style="color: #c5c6c7;">Total Score: ${totalAll}</p>
